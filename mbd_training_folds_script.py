...
def train(
    ...
    if train_loader==None:
        train_loader, val_loader, epoch_len, train_val_loader, train_ds, val_ds = load_data(args, training_key=args.training_key, validation_key=args.validation_key)
    # 3. build model
    ...
    if wandb.run.resumed:
        ...
        model_weights_path = f'./trained_models/{run_name}_{run_id}/model_mbd_{run_name}_{run_id}_last.pt'
        model_weights = torch.jit.load(model_weights_path) 
        detector.network.load_state_dict(model_weights.state_dict())
...
def main():
    ...
    train_loader, val_loader, epoch_len, train_ds, val_ds = load_data(args, training_key=training_key, validation_key=validation_key)
