...
def train(
    ...
    if args.patch_size and args.batch_size and args.lr and args.conv1_t_stride:
    ...
    #choose number of validation images to save
    val_epochs = np.array([x+1 for x in range(max_epochs) if (x+1)%val_interval==0])
    
    if len(val_epochs) < nr_val_images:
        nr_imgs = len(val_epochs)
    else:
        nr_imgs = nr_val_images
        
    val_img_epochs = val_epochs[np.linspace(0, len(val_epochs)-1, num=nr_imgs, dtype=int)]
    ...
    if nr_imgs>0:
        os.makedirs(f'./validation_imgs/{run_name}_{run_id}/', exist_ok=True)
    ...
    detector.eval()
    # mode.value = 0 #IMPORTANT -> SETS THE TRANSFORMS IN DATALOADER TO VALIDATION TRANSFORMS
    val_outputs_all = []
    val_targets_all = []
    start_time = time.time()
    with torch.no_grad():
        for val_data in train_val_loader:
            # if all val_data_i["image"] smaller than args.val_patch_size, no need to use inferer
            # otherwise, need inferer to handle large input images.
            use_inferer = not all(
                [val_data_i["image"][0, ...].numel() < np.prod(args.val_patch_size) for val_data_i in val_data]
    ...
    # mode.value = 1 #IMPORTANT -> SETS THE TRANSFORMS IN DATALOADER BACK TO TRAINING TRANSFORMS
    ...
    wandb.log({"val_metric (train)": val_epoch_metric,
               f'AP_IoU_0.10_MaxDet_{detections_per_img} (train)': ap_iou_10,
               f'AP_IoU_0.50_MaxDet_{detections_per_img} (train)': ap_iou_50,
               f'mAP_IoU_0.10_0.50_0.05_MaxDet_{detections_per_img} (train)': map_iou,
               f'AR_IoU_0.10_MaxDet_{detections_per_img} (train)': ar_iou_10,
               f'AR_IoU_0.50_MaxDet_{detections_per_img} (train)': ar_iou_50,
               f'mAR_IoU_0.10_0.50_0.05_MaxDet_{detections_per_img} (train)': mar_iou,
               "epoch": epoch+1
              })

    with open(f"{args.model_path.split('/')[0]}/{run_name}_{run_id}/train_coco_metric_{run_name}_{run_id}_last.json", "w") as f:
        json.dump(val_epoch_metric_dict, f)

    results_path = f"{args.model_path.split('/')[0]}/{run_name}_{run_id}/train_results_metric_{run_name}_{run_id}_last.pkl"
    pd.to_pickle(results_metric_dict, results_path)
...
def main():
    ...
    global args, env_dict, config_dict, train_val, epoch_len, mode
    # Define args
    ...
    for modality, name, with_binary in zip([['T1', 'ADC', 'b1000'], ['T1', 'ADC', 'b1000', 'skeleton']], ['T1_ADC_b1000', 'T1_ADC_b1000_skeleton'], [False, True]):
        ...
        env_dict, config_dict, train_val = prepare_json_files(
            args=args,                                
        ...
        train_loader, val_loader, epoch_len, train_val_loader, _ = load_data(args, training_key=training_key, validation_key=validation_key)

        # Define pretrained model
        # folder_path = './trained_models'
        # filename = 'model_mbd_multi_down_honest-bird-94_last.pt'
        # pretrained_path = os.path.join(folder_path, filename)
